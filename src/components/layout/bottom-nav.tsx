'use client';

import { usePathname, useRouter } from 'next/navigation';
import { Home, Trophy, CircleDot, User } from 'lucide-react';

const itens = [
  { href: '/inicio', label: 'Home', icone: Home },
  { href: '/grupos', label: 'Grupos', icone: Trophy },
  { href: '/jogos', label: 'Jogos', icone: CircleDot },
  { href: '/minha-conta', label: 'Conta', icone: User },
];

export function BottomNav() {
  const pathname = usePathname();
  const router = useRouter();

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-30 bg-fundo/90 backdrop-blur-lg border-t border-white/[0.06] safe-area-bottom" data-testid="bottom-nav">
      <div className="mx-auto max-w-[480px] flex items-center justify-around py-2 px-2">
        {itens.map((item) => {
          const ativo = pathname === item.href || pathname?.startsWith(item.href + '/');
          const Icone = item.icone;

          return (
            <button
              key={item.href}
              onClick={() => router.push(item.href)}
              className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-lg transition-all ${
                ativo
                  ? 'text-primaria'
                  : 'text-texto/35 hover:text-texto/60'
              }`}
              aria-label={item.label}
              aria-current={ativo ? 'page' : undefined}
              data-testid={`nav-btn-${item.label.toLowerCase()}`}
            >
              <Icone size={20} strokeWidth={ativo ? 2.5 : 1.5} />
              <span className={`text-[10px] ${ativo ? 'font-semibold' : 'font-normal'}`}>
                {item.label}
              </span>
            </button>
          );
        })}
      </div>
    </nav>
  );
}
